...
static int decode_open_type(const uint8_t *buf, int limit, int *len, const uint8_t **p_object, int *p_num_octets)
{
	...
	*p_num_octets = 0;
	...
	/* We do not deal with fragments, so there is no point in looping through them. Just say that something
	   fragmented is bad. */
	if (decode_length(buf, limit, len, &octet_cnt) != 0) return -1;
	*p_num_octets = octet_cnt;
	if (octet_cnt > 0) {
		/* Make sure the buffer contains at least the number of bits requested */
...
static int encode_open_type(uint8_t *buf, int *len, const uint8_t *data, int num_octets)
{
	...
	for (octet_idx = 0;; num_octets -= enclen, octet_idx += enclen) {
		if ((enclen = encode_length(buf, len, num_octets)) < 0) return -1;
		if (enclen > 0) {
			memcpy(&buf[*len], &data[octet_idx], enclen);
			*len += enclen;
		}
		if (enclen >= num_octets) break;
	}
...
int udptl_rx_packet(udptl_state_t *s, const uint8_t buf[], int len)
{
	...
	if ((stat = decode_open_type(buf, len, &ptr, &msg, &msg_len)) != 0) return -1;
	/* Decode error_recovery */
	if (ptr + 1 > len) return -1;
	/* Our buffers cannot tolerate overlength packets */
	if (msg_len > LOCAL_FAX_MAX_DATAGRAM) return -1;
	...
	if (msg_len > 0) memcpy(s->rx[x].buf, msg, msg_len);
	...
	if ((buf[ptr++] & 0x80) == 0) {
		...
		total_count = 0;
		do {
			if ((stat = decode_length(buf, len, &ptr, &count)) < 0) return -1;
			...
			for (i = 0; i < count; i++) {
				if (decode_open_type(buf, len, &ptr, &bufs[total_count + i], &lengths[total_count + i]) != 0) return -1;
			}
			total_count += count;
		} while (stat > 0);
		...
		if (seq_no > s->rx_seq_no) {
			...
			for (i = total_count; i > 0; i--) {
				if (seq_no - i >= s->rx_seq_no) {
					...
					if (lengths[i - 1] > 0) memcpy(s->rx[x].buf, bufs[i - 1], lengths[i - 1]);
		...
		/* Decode the elements */
		for (i = 0; i < entries; i++) {
			if ((stat = decode_open_type(buf, len, &ptr, &data, &s->rx[x].fec_len[i])) != 0) return -1;
			if (s->rx[x].fec_len[i] > LOCAL_FAX_MAX_DATAGRAM) return -1;

			/* Save the new FEC data */
			if (s->rx[x].fec_len[i]) memcpy(s->rx[x].fec[i], data, s->rx[x].fec_len[i]);
#if 0
		...
		for (l = x; l != ((x - (16 - span * entries)) & UDPTL_BUF_MASK); l = (l - 1) & UDPTL_BUF_MASK) {
			if (s->rx[l].fec_len[0] <= 0) continue;
			for (m = 0; m < s->rx[l].fec_entries; m++) {
				...
				for (which = -1, k = (limit - s->rx[l].fec_span * s->rx[l].fec_entries) & UDPTL_BUF_MASK; k != limit;
					...
					if (s->rx[k].buf_len <= 0) which = (which == -1) ? k : -2;
				}
				if (which >= 0) {
					/* Repairable */
...
int udptl_build_packet(udptl_state_t *s, uint8_t buf[], const uint8_t msg[], int msg_len)
{
	...
	/* UDPTL cannot cope with zero length messages, and our buffering for redundancy limits their
	   maximum length. */
	if (msg_len < 1 || msg_len > LOCAL_FAX_MAX_DATAGRAM) return -1;
	...
	if (encode_open_type(buf, &len, msg, msg_len) < 0) return -1;
	...
	case UDPTL_ERROR_CORRECTION_NONE:
		...
		if (encode_length(buf, &len, 0) < 0) return -1;
		break;
	case UDPTL_ERROR_CORRECTION_REDUNDANCY:
		...
		/* The number of entries will always be small, so it is pointless allowing
		   for the fragmented case here. */
		if (encode_length(buf, &len, entries) < 0) return -1;
		/* Encode the elements */
		for (m = 0; m < entries; m++) {
			previous_len = len;
			j = (entry - m - 1) & UDPTL_BUF_MASK;
			if (encode_open_type(buf, &len, s->tx[j].buf, s->tx[j].buf_len) < 0) return -1;
			...
			if (len > s->far_max_datagram_size) {
				len = previous_len;
				if (encode_length(buf, &len_before_entries, m) < 0) return -1;
				break;
			}
		}
		break;
	case UDPTL_ERROR_CORRECTION_FEC:
		...
		if (seq < s->error_correction_span * s->error_correction_entries) {
			...
			if (seq < s->error_correction_span) span = 0;
		}
		/* Encode the error recovery type */
		buf[len++] = 0x80;
		...
		for (m = 0; m < entries; m++) {
			...
			for (i = (limit - span * entries) & UDPTL_BUF_MASK; i != limit; i = (i + entries) & UDPTL_BUF_MASK) {
				if (high_tide < s->tx[i].buf_len) {
					for (j = 0; j < high_tide; j++) fec[j] ^= s->tx[i].buf[j];
					for (; j < s->tx[i].buf_len; j++) fec[j] = s->tx[i].buf[j];
					high_tide = s->tx[i].buf_len;
				} else {
					for (j = 0; j < s->tx[i].buf_len; j++) fec[j] ^= s->tx[i].buf[j];
				}
			}
			if (encode_open_type(buf, &len, fec, high_tide) < 0) return -1;
...
int udptl_set_error_correction(udptl_state_t *s, int ec_scheme, int span, int entries)
{
	...
	if (span >= 0) s->error_correction_span = span;
	if (entries >= 0) s->error_correction_entries = entries;
	return 0;
}
...
{
	if (ec_scheme) *ec_scheme = s->error_correction_scheme;
	if (span) *span = s->error_correction_span;
	if (entries) *entries = s->error_correction_entries;
	return 0;
}
...
int udptl_get_local_max_datagram(udptl_state_t *s) { return s->local_max_datagram_size; }
...
int udptl_get_far_max_datagram(udptl_state_t *s) { return s->far_max_datagram_size; }
						  ...
						  udptl_rx_packet_handler_t rx_packet_handler, void *user_data)
{
	...
	if (rx_packet_handler == NULL) return NULL;

	if (s == NULL) {
		if ((s = (udptl_state_t *)malloc(sizeof(*s))) == NULL) return NULL;
	}
	memset(s, 0, sizeof(*s));
...
int udptl_release(udptl_state_t *s) { return 0; }
